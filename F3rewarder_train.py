...
if __name__ == '__main__':
    name = 'F3rewarder' + datetime.now().strftime('%Y%m%d%H%M')
    ...
    backbone.load_state_dict(torch.load('model/F3HRNetlog3.pth'))
    backbone.eval()
        ...
        with autocast(enabled=True):
            if idx < config.start_timing:
                ...
                for i in range(B):
                    ...
                    p = logCube[i, :, :, coordinates_x, coordinates_y]
                    p_list = torch.where(p[0] != -999.)

                    r = logCube[i, :, int(p_list[0][0]):int(p_list[0][-1])+1, coordinates_x, coordinates_y][None]
                    r = F.interpolate(r, size=(400,), mode='linear', align_corners=False)
                    real_labels.append(r)

                    g = togenerater_x_lb[i, :, int(p_list[0][0]):int(p_list[0][-1]) + 1, coordinates_x, coordinates_y][None]
                    g = F.interpolate(g, size=(400,), mode='linear', align_corners=False)
                    togenerater_feats.append(g)

                    # predicted_labels.append(logits_x_lb[i,:,int(p_list[0][0]):int(p_list[0][-1])+1,coordinates_x, coordinates_y])
