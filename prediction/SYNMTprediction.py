...
all = z_score_clip(np.load('../data/denoise_seismic.npy'), 3.2)
print(all.shape)
val = all
all = all[None,None]
val = torch.from_numpy(val)
val = F.interpolate(val[None,None], (400, 496, 496), mode='trilinear', align_corners=True)

with autocast(enabled=True):
# with autocast('cpu'):
    val_output, x_togenerater = model(val, val)[0, 0, :]
val_output = val_output.detach().numpy()
np.save("../result/SYNlog16fixmatchmodel.npy", val_output)
